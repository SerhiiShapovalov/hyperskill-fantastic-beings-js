...
let moves = 1;
let score = 0;
const winCondition = { zouwu: 3, kelpie: 0 };
const collectedCreatures = { zouwu: 0, kelpie: 0 };

function updateStatusBar() {
    document.getElementById("moves-value").textContent = String(moves);
    document.getElementById("score-value").textContent = score;
    document.querySelector(".zouwu").textContent = String(winCondition.zouwu - collectedCreatures.zouwu);
    document.querySelector(".kelpie").textContent = String(winCondition.kelpie - collectedCreatures.kelpie);
}

function checkWinOrLose() {
    const footer = document.getElementById("game-footer");

    if (collectedCreatures.zouwu >= winCondition.zouwu && collectedCreatures.kelpie >= winCondition.kelpie) {
...
function checkForMatches() {
    ...
    const rows = [];
    const gridCols = parseInt(getComputedStyle(document.getElementById('map')).getPropertyValue('--cols-count'));
...
function findMatches(rows) {
    ...
    rows.forEach((row, rowIndex) => {
        ...
        for (let colIndex = 0; colIndex < row.length; colIndex += 1) {
            ...
            if (nextCell && currentCell.dataset.being === nextCell.dataset.being && currentCell.dataset.being) {
                matchLength += 1;
            } else {
                if (matchLength >= 3) {
                    console.log(`Horizontal match: ${matchLength} creatures in row ${rowIndex}`);
    ...
    for (let colIndex = 0; colIndex < rows[0].length; colIndex += 1) {
        ...
        for (let rowIndex = 0; rowIndex < rows.length; rowIndex += 1) {
            ...
            if (nextCell && currentCell.dataset.being === nextCell.dataset.being && currentCell.dataset.being) {
                ...
                if (matchLength >= 3) {
                    console.log(`Vertical match: ${matchLength} creatures in column ${colIndex}`);
                    for (let i = 0; i < matchLength; i += 1) {
                        matches.push({
                            row: rowIndex - i,
                            col: colIndex
